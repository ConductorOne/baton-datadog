...
Each Datadog team exposes two entitlements, **Member** and **Admin**. Granting **Admin** to a user who is already a member of the team promotes them to team admin. Granting **Member** to a user who is already a team admin leaves them as admin — the connector does not demote on grant.

Requesting a role or team membership the user already holds completes successfully without making a change, so a retried or duplicate request does not fail.
